 ...
 * Fetch individual reactions for a post, including reactor profiles (name, headline/title, picture, profile URL).
 * Currently only supported for **LinkedIn organization/company page** posts.
 * LinkedIn restricts reaction data for personal profiles (r_member_social_feed is a closed permission).
 *
 */
export const getPostReactions = <ThrowOnError extends boolean = false>(options: OptionsLegacyParser<GetPostReactionsData, ThrowOnError>) => {
    return (options?.client ?? client).get<GetPostReactionsResponse, GetPostReactionsError, ThrowOnError>({
        ...options,
